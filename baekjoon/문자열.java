...
//		maxcharat();
	...
	private static void groupString() throws IOException {
	...
	private static void Krwawhat() throws IOException {
		...
		for (int i = 0; i < str.length(); i++) {
			...
			if (i < str.length() - 1) {
				...
				if (str.charAt(i) == 'd' && str.charAt(i + 1) == 'ž') {
					cnt++;
					i++;
	...
	private static void blankspase() throws IOException {
		...
		// ""빈 문자열은 null이 아니고 ""로 인스턴스화 된 문자열이기 때문에 split한 결과를 담은 배열의 길이는 0이 아니라 1이 되게
		// 된다.
